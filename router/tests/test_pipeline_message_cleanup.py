"""Tests for Phase 3: delete-before / save-after message_id in RouterPipeline."""
import asyncio
from unittest.mock import AsyncMock, MagicMock

import pytest

from app.routing.models import RouterCommand, RouterResult
...
def _make_command(command_type: str = "player_action") -> RouterCommand:
...
def _make_result(command_type: str = "player_action") -> RouterResult:
    return RouterResult(
        success=True,
        command_type=command_type,
        message="ok",
...
) -> tuple[RouterPipeline, MagicMock, MagicMock, MagicMock]:
...
def test_tutorial_group_saves_group_open_hint():
    normalizer = MagicMock()
    normalizer.normalize.return_value = RouterCommand(
        update_id=1,
        chat_id="chat-1",
        chat_type="group",
        actor_telegram_id="tg-1",
