...
class BaseCommandHandlers:
    ...
    ) -> GameServiceEnvelope | RouterResult:
        try:
    ...
    async def _check_turn_owner_before_action(self, command: RouterCommand) -> RouterResult | None:
        if self._session_context_store is None:
            return None
    ...
    async def _check_available_moves_before_action(self, command: RouterCommand) -> RouterResult | None:
        if self._session_context_store is None:
            return None
    ...
    @staticmethod
    def _request_failed_result(command: RouterCommand) -> RouterResult:
        return RouterResult(
            success=False,
            command_type=command.command_type,
