...
from app.game_client.models import GroupPlayerStopRequest
...
class SessionCommandHandlers(BaseCommandHandlers):
    ...
    async def handle_group_open(self, command: RouterCommand) -> RouterResult:
        ...
        if isinstance(envelope_or_error, RouterResult):
    ...
    async def handle_group_stop(self, command: RouterCommand) -> RouterResult:
        if command.actor_telegram_id is None:
            return RouterResult(
                success=False,
                command_type=command.command_type,
